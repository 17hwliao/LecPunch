...
import { AUTH_EXPIRED_EVENT, ApiError, acknowledgeNotification, checkIn, checkOut, clearToken, enterAdminPreview, fetchAttendance, fetchCurrentUser, fetchNotifications, fetchPoints, fetchTeamActiveAttendance, fetchTeamWeeklyStats, getApiBaseUrl, login, readToken, startNotificationStream, updatePassword, updateProfile } from '@/lib/api';
...
export const App = () => {
  ...
  if (!user) return <LoginScreen loading={loading} notice={notice} onLogin={async (username, password) => { setLoading(true); try { setUser(await login(username, password)); await refresh(); } catch (error) { setNotice(error instanceof Error ? error.message : '登录失败'); setLoading(false); } }} onOpenPreview={async () => { setLoading(true); try { setUser(enterAdminPreview()); await refresh(); } catch (error) { setNotice(error instanceof Error ? error.message : '无法打开本地演示'); } finally { setLoading(false); } }} />;
